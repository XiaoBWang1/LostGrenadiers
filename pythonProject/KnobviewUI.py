...
class MapSearchWindow(PyQt5.QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.cvButton.clicked.connect(self.run_kv2)

        self.cvButton = PyQt5.QtWidgets.QPushButton("3rd Floor", self)
        self.cvButton.move(20, 80)
        self.cvButton.clicked.connect(self.run_kv3)
    ...
    def run_kv2(self):
        subprocess.run(['python', 'KnobviewUI2.py'])
        sys.exit(app.exec_())

    def run_kv3(self):
        subprocess.run(['python', 'KnobviewUI3.py'])
        sys.exit(app.exec_())
